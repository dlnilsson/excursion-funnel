...
func TestRoot_RegistersCommandsAndCompletion(t *testing.T) {
	...
	for _, name := range []string{"serve", "hub", "version", "usage", "sessions", "tools", "requests", "inspect", "completion"} {
...
func TestRoot_SubcommandHelpSucceeds(t *testing.T) {
	for _, args := range [][]string{{"serve", "--help"}, {"usage", "--help"}, {"sessions", "--help"}, {"sessions", "today", "--help"}, {"sessions", "week", "--help"}, {"tools", "--help"}, {"requests", "--help"}} {
